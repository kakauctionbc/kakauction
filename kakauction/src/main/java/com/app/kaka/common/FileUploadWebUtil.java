		...
		public String getUploadPath(HttpServletRequest request, int uploadType){
			...
			if(type.equals("test")){
				...
				if(uploadType==PDS_UPLOAD){
					//자료실 업로드
					realtPath= fileUploadProps.getProperty("file.upload.path.test");		
				}else{
					//상품 등록시 파일 이미지 업로드
					realtPath=fileUploadProps.getProperty("imageFile.upload.path.test");
				}
				logger.info("테스트 경로 ={}", realtPath);
			}else{
				//실제 배포하는 경우 => 실제 경로를 구한다
				if(uploadType==PDS_UPLOAD){
					realtPath= fileUploadProps.getProperty("file.upload.path");
				}else{
					realtPath= fileUploadProps.getProperty("imageFile.upload.path");
				}
				logger.info("실제 배포시 경로 ={}", realtPath);
				//물리적인 경로 구하기
				realtPath=
						request.getSession().getServletContext().getRealPath(realtPath);
				logger.info("실제 배포시 물리적 경로 ={}", realtPath);
			}//if
			return realtPath;
		}
