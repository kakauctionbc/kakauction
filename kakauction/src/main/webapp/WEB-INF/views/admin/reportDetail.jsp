			...
			<div class="divForm">
				<form action="<c:url value='/report/handle.do'/>" method="post" id="reportFrm" name="reportFrm">
					...
					<div class="lastDiv">			
						...
						<br>
						<c:if test="${reportVo.originType == 1}">
							<a href="<c:url value='/freeboard/detail.do?freeboardNo=${reportVo.originNo }'/>" style="float: right;">원글 보러가기</a>
						</c:if>
						<c:if test="${reportVo.originType == 2}">
							<a href="<c:url value='/auction/auctiongo.do?auctionNo=${reportVo.originNo }'/>" style="float: right;">원글 보러가기</a>
						</c:if>
					</div>
					<div class="center">
